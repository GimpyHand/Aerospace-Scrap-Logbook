...
def load_data():
    if os.path.exists(csv_file_path):
        ...
        for i, col in enumerate(df.columns):
            header = tk.Label(header_frame, text=col, borderwidth=1, relief="solid", bg='lightblue', anchor='center', padx=5, pady=5, font=('Arial', 10, 'bold'))
            header.grid(row=0, column=i, sticky="nsew")
            header_frame.grid_columnconfigure(i, minsize=column_widths.get(col, 100))  # Set minimum column width
...
root.grid_rowconfigure(1, weight=0)  # Header row
root.grid_rowconfigure(2, weight=3)  # Table row
